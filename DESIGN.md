...
- 紹介文や機能一覧ではなく、実際の主要画面をブラウザまたはスクリーンショットで見る。
- 原則として、各参考元につき今回の主要画面に近いDesktop画面1つを見ればよい。
- loading、empty、error、すべての操作状態、すべての画面幅を網羅的に調べる必要はない。
- HTMLソース、DOM、Computed Styles、CSS variablesを確認できる場合は、全体を解析せず、フォント、主要領域の比率、余白、背景、境界、角丸、影など、第一印象を左右する判断に必要な範囲だけ見る。
- HTMLやCSSを確認できない場合も、スクリーンショットから十分に判断できるなら作業を止めない。

生のHTMLソースは現代のWebアプリでは設計判断を読み取りにくい場合がある。必要な値を調べるときは、可能ならブラウザ上のDOMとComputed Stylesを優先する。
